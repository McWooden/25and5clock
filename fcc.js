function App() {
    ...
    return (
        ...
        <div id="timer-label" className="container bg-white shadow rounded text-center p-0 pt-2 rounded">
            <div className="shadow rounded py-2 mx-4 border border-info">
                <p className="fs-3 mb-3">{mode}</p>
    ...
    return (
        <div id={`${name}-label`} className={`${color} shadow m-3 rounded p-4 control flex-fill`}>
            ...
            <div className="d-flex justify-content-center mt-3">
                <div className="btn btn-primary" id={`${name}-decrement`} onClick={() => ment('DECREMENT')}>
                    <i className="fa-solid fa-caret-down fs-2"></i>
                </div>
                <p className="mx-3 my-0 fs-2" id={`${name}-length`}>{number}</p>
                <div className="btn btn-primary" id={`${name}-increment`} onClick={() => ment('INCREMENT')}>
